...
class DatabaseBackend(BaseMemoryBackend):
    ...
    async def store(self, chunk: MemoryChunk) -> bool:
        ...
        try:
            embedding_blob = (
                json.dumps(chunk.embedding).encode("utf-8") if chunk.embedding else None
            )
...
class MemoryManager:
    ...
    async def _check_capacity(self, memory_type: MemoryType):
        """检查并清理超出容量的记忆"""
        # 简单的清理策略：删除最不重要的记忆
        all_memories = await self.backend.search_by_tags(
            [], top_k=self.capacity_limits[memory_type] * 2
        )
    ...
    async def retrieve_memories(
        self, query: str, memory_type: MemoryType = None, top_k: int = 5
    ) -> List[MemoryChunk]:
        """检索记忆"""
    ...
    async def update_memory(
        self, memory_id: str, content: str = None, importance: float = None, tags: List[str] = None
    ) -> bool:
        """更新记忆"""
...
class ConversationMemory:
    ...
    async def get_conversation_history(self, limit: int = 50) -> List[Message]:
        ...
        memories = await self.memory_manager.search_by_tags(
            ["message", self.conversation_id], top_k=limit
        )
