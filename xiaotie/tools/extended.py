    ...
    def parameters(self) -> dict:
        return {
            ...
            "properties": {
                ...
                "command": {
                    "type": "string",
                    "description": "启动命令（用于start操作）"
                }
            },
            "required": ["action"]
        }

    async def execute(self, action: str, process_name: Optional[str] = None, command: Optional[str] = None) -> ToolResult:
        try:
            import psutil
            ...
            if action == "list":
                ...
                return ToolResult(
                    success=True,
                    content=f"已启动进程，PID: {process.pid}"
                )
...
class NetworkTool(Tool):
    ...
    def parameters(self) -> dict:
        return {
            ...
            "properties": {
                ...
                "host": {
                    "type": "string",
                    "description": "目标主机（用于ping和port_scan）"
                },
                "ports": {
                    "type": "array",
                    "items": {"type": "integer"},
    ...
    async def execute(self, action: str, host: Optional[str] = None, ports: Optional[List[int]] = None) -> ToolResult:
        try:
            if action == "ping" and host:
                ...
                return ToolResult(
                    success=True,
                    content=json.dumps(conn_list, indent=2)
                )
                ...
                return ToolResult(
                    success=True,
                    content=f"主机 {host} 上开放的端口: {open_ports}"
                )
